...
class obj_fun():
    def __init__(self):
        self.choices = ['rosen', 'rosen_3', 'rosen_4', 'rosen_6', 'branin', 'camel_3', 'sphere', 'sphere_3', 'sphere_4', 'sphere_6', 'mccor', 'hart_3', 'hart_4', 'hart_6']
        
    def get_obj_fun(self, choice, x, params):
        if choice == 'rosen':
...
def max_score_ours(X_0, X_known, y_known, X_grid, regressor, params):
    beta = params
...
def max_score_PI(X_0, X_known, y_known, X_grid, regressor, params):
    tradeoff = params
...
def max_score_UCB(X_0, X_known, y_known, X_grid, regressor, params):
    beta = params
...
def max_score_EI(X_0, X_known, y_known, X_grid, regressor, params):
    tradeoff = params
    ...
    def PI(self, regressor, X_grid, X_known, y_known, n_instances, params, LB, UB):
        ...
        for j in range(0, n_instances):
            ...
            for k in range(0,25):
                ...
                for col in range(dims):
                    x0[col] = np.random.uniform(LB[col]+0.001, UB[col]-0.001)
                res = minimize(max_score_PI, x0, args= (X_known, y_known, X_grid, regressor, params), method='Powell',bounds = bnds)
                best_options[k] = res.x
                best_scores[k] = res.fun
    ...
    def EI(self, regressor, X_grid, X_known, y_known, n_instances, params, LB, UB):
        ...
        for j in range(0, n_instances):
            ...
            for k in range(0,25):
                ...
                for col in range(dims):
                    x0[col] = np.random.uniform(LB[col]+0.001, UB[col]-0.001)
                res = minimize(max_score_EI, x0, args= (X_known, y_known, X_grid, regressor, params), method='Powell',bounds = bnds)
                best_options[k] = res.x
                best_scores[k] = res.fun
    ...
    def UCB(self, regressor, X_grid, X_known, y_known, n_instances, params, LB, UB):
        ...
        for j in range(0, n_instances):
            ...
            for k in range(0,25):
                ...
                for col in range(dims):
                    x0[col] = np.random.uniform(LB[col]+0.001, UB[col]-0.001)
                res = minimize(max_score_UCB, x0, args= (X_known, y_known, X_grid, regressor, params), method='Powell',bounds = bnds)
                best_options[k] = res.x
                best_scores[k] = res.fun
...
def get_init_known(ob_name, init, dim, seed, params):
...
def alg_test(alg_info, all_data, obj_fun_name, data_size, alt_params, date):
    ...
    for alt in range(len(alt_params)):
        ...
        for row in range(len(X_tot)):
            y_grid[row] = obj.get_obj_fun(obj_fun_name, X_tot[row,:], alt_params[alt,:])
        ...
        np.savetxt('Top_val_%s_%s_%s_%s_%s_%s.csv' % (phys_dec, acq, kern, obj_fun_name, alt, date), top_data, delimiter=',')
        np.savetxt('Min_val_%s_%s_%s_%s_%s_%s.csv' % (phys_dec, acq, kern, obj_fun_name, alt, date), min_data, delimiter=',')
        ...
        X_known, y_known = get_init_known(obj_fun_name, init_points_num, dims, rnd_seed, alt_params[alt,:])
        X_known = np.reshape(X_known, (-1,dims))
        y_known = np.reshape(y_known, (-1,1))                                                        
        
        test_tot_preds = np.zeros((tot_batches, data_size))
        x_vals = np.zeros((tot_batches, len(get_obj_info(obj_fun_name, 'params'))))
        
        phys_params = get_obj_info(obj_fun_name, 'params')
        for j in range(0, tot_batches):
            if phys_dec == 0:
                pass
            elif phys_dec == 1:
                X_used, y_used = get_used_data(X_known, y_known, obj_fun_name, tot_points, phys_params)
                X_used = np.reshape(X_used, (-1,dims))
                y_used = np.reshape(y_used, (-1,1))
            ...
            # X_tot, X_known, y_known, batch, a, b, d, new_eps, kern, acq, obj_fun_name
            X_known = np.append(X_known, X_new)
            X_known = np.reshape(X_known, (-1,dims))
            obj = obj_fun()
            y_known_new = np.zeros((X_new.shape[0],1))
            for row in range(X_new.shape[0]):
                y_known_new[row] = obj.get_obj_fun(obj_fun_name, X_new[row,:], alt_params[alt,:])
            ...
            if phys_dec == 0:
                ...
                x0 = phys_params
                temp, phys_params = non_lin(fun, x0, X_known, y_known, obj_fun_name) 
                x_vals[j,:] = phys_params

                if j == tot_batches -1:
                    np.savetxt('Obj_params_%s_%s_%s_%s_%s_%s.csv' % (phys_dec, acq, kern, obj_fun_name, alt, date), x_vals, delimiter=',')

            if j == tot_batches - 1:
                np.savetxt('Known_vals_%s_%s_%s_%s_%s_%s.csv' % (phys_dec, acq, kern, obj_fun_name, alt, date), np.concatenate((X_known,y_known),axis=1), delimiter=',')
...
# 'Alg_test.csv' contains information about which algorithm and objective function to test
alg_info = pd.read_csv('Alg_test.csv', sep=',', header=0)
alg_info = alg_info.values
date = alg_info[0,5]
ob_name = alg_info[0,3]
# Generate data with physics model
ob_fun = obj_fun()
LB = get_obj_info(ob_name, 'LB')
UB = get_obj_info(ob_name, 'UB')
num = alg_info[0,4]
dim = get_obj_info(ob_name, 'dims')
total_size = num**dim

params =get_obj_info(ob_name, 'params')

variance = get_obj_info(ob_name, 'param var')

# Original data
X = create_grid(LB, UB, num, dim, 1)
# LB, UB, num, dim, num_diff
Z = np.zeros((len(X), 1))
ind =0
for x in X:
    Z[ind] = ob_fun.get_obj_fun(ob_name, x, params)
    ind = ind + 1
...
np.savetxt('alt_params_%s_%s.csv' % (ob_name, date), alt_params, delimiter=',')
...
for row in alg_info:
    ...
    alg_test(row, all_data, ob_name, total_size, alt_params, date)

    mean_error, std_error = error_calc(phys_dec, acq_fun, kern, ob_name, date)
    mean_dist, std_dist = dist_calc(phys_dec, acq_fun, kern, ob_name, date)
    np.savetxt('Error_vals_%s_%s_%s_%s_%s.csv' % (phys_dec, acq_fun, kern, ob_name, date), mean_error, delimiter=',')
    np.savetxt('Error_std_%s_%s_%s_%s_%s.csv' % (phys_dec, acq_fun, kern, ob_name, date), std_error, delimiter=',')
    np.savetxt('Dist_vals_%s_%s_%s_%s_%s.csv' % (phys_dec, acq_fun, kern, ob_name, date), mean_dist, delimiter=',')
    np.savetxt('Dist_std_%s_%s_%s_%s_%s.csv' % (phys_dec, acq_fun, kern, ob_name, date), std_dist, delimiter=',')
