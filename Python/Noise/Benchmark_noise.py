...
def get_init_known(ob_name, init, dim, seed, params, range_z, noise):
    ...
    for j in np.arange(int(init)):
        y_choices[j] = obj.get_obj_fun(ob_name, choices[j,:], params) + (random.random()*2*range_z-range_z)*noise
...
def alg_test(alg_info, all_data, obj_fun_name, data_size, alt_params, range_z, noise):
    ...
    for alt in range(len(alt_params)):
        ...
        X_known, y_known = get_init_known(obj_fun_name, init_points_num, dims, rnd_seed, alt_params[alt,:], range_z[alt], noise)
        X_known = np.reshape(X_known, (-1,dims))
        y_known = np.reshape(y_known, (-1,1))                                                        
        ...
        for j in range(0, tot_batches):
            ...
            for row in range(X_new.shape[0]):
                y_known_new[row] = obj.get_obj_fun(obj_fun_name, X_new[row,:], alt_params[alt,:]) + (random.random()*2*range_z[alt]-range_z[alt])*noise
...
range_z = np.zeros((num_alts,1))
for j in range(num_alts):
    ind =0
    Z_alt = np.zeros((len(X), 1))
    for x in X:
        Z_alt[ind] = ob_fun.get_obj_fun(ob_name, x, alt_params[j,:])
        ind = ind + 1
    range_z[j] = np.amax(Z_alt) - np.amin(Z_alt)
...
for row in alg_info:
    ...
    alg_test(row, all_data, ob_name, total_size, alt_params, range_z, noise)
